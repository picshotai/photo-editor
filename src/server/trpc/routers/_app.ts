...
export const appRouter = router({
  ...
  generateImageStream: publicProcedure
    ...
    .subscription(async function* ({ input, signal, ctx }) {
      try {
        ...
        if (!images?.[0]?.url) {
          yield tracked(`${generationId}_error`, {
            type: "error",
            error: "No image produced by nano-banana/edit",
          });
          return;
        }

        // Final completion event with image URL
        yield tracked(`${generationId}_complete`, {
          type: "complete",
          imageUrl: images[0].url,
        });
      } catch (error) {
        console.error("Error in nano-banana edit stream:", error);
        yield tracked(`error_${Date.now()}`, {
          type: "error",
    ...
    // legacy flux-kontext-lora streaming removed
});
